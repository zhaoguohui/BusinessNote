...
NSString * const COM_CHANGE                         = @"c";         // 服务器发送的远端变化消息
NSString * const COM_CHANGE_VERSION                 = @"cv";        // 服务器变化的版本
NSString * const COM_ENTITY                         = @"e";
NSString * const COM_ERROR                          = @"?";
NSString * const COM_LOG                            = @"log";       // 服务器发送的日志消息
NSString * const COM_INDEX_STATE                    = @"index";
NSString * const COM_HEARTBEAT                      = @"h";         // 服务器发送的心跳消息
...
- (void)webSocket:(SPWebSocket *)webSocket didReceiveMessage:(id)message {
    ...
    NSLog(@"didReceiveMessage:%@", components);
    ...
    // Message: Heartbeat
    if ([channelStr isEqualToString:COM_HEARTBEAT]) { // 发送的心跳消息
        return;
    }
    
    // Message: LogLevel
    if ([channelStr isEqualToString:COM_LOG]) { // 发送的打开或关闭日志的消息
    ...
    } else if ([command isEqualToString:COM_CHANGE]) {          // 服务器端发生变化的消息
        // 将变化的json信息转化为对象
    ...
    } else if ([command isEqualToString:COM_ERROR]) { // 出现错误的消息
